...
### 5.3 质感
- 背景：**用户自制甘道夫图**（AI 生成/自绘，无版权争议；压暗 58% + 缩放 2560 + JPEG 233KB，base64 内联）+ 蓝黑遮罩 0.16 + 9 颗程序化金色星点
- 字体：Cinzel 标题（OFL 自托管 base64）+ Noto Sans SC 正文（OFL，后续接）
- 边框：鎏金描边、圆角柔和、金色分隔线/星辉、符文分隔 SVG（自制）
- 聊天气泡：夜蓝面板（80% 不透明）+ 金褐描边

## 6. 可读性红线（开发工具铁律）
- 正文对比度 ≥ 4.5:1（WCAG AA）——实测 11/11 全过（正文 15.81:1、金色 12.28:1、按钮 11.58:1）
- 文字容器（气泡/代码/输入）不透明度 ≥ 0.76 保可读；空白区透背景
- 背景图压暗至 avgLum ~20 档；代码/文字区域不花哨字体
