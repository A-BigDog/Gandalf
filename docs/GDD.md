...
> 主题仍在迭代（当前浅色实验版）——具体数值以 `tokens.ts`（透明度/颜色）和 `theme.css.ts`（背景/控件）为准，改完跑 `plugin/build.cmd` 一键构建。

- **配色**：浅色主题——白色面板（半透明/全透可调）+ 甘道夫背景图（cover，位置可调）；**字色 DSH 默认**（不覆盖文字 token）
- **字体**：`--dsw-font-family` → 霞鹭文楷等宽（本地）；代码区保持默认等宽
- **控件**：发送按钮五芒星（限定 composer）、新会话/回到底部透明、设置面板/选择框白色可调
