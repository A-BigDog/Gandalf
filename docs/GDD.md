...
- 仅覆盖 23 个表面 token 为**半透明**（取默认色 + alpha 0.58–0.94），让甘道夫背景图透出
- 文字/品牌/状态色全部默认（正文 #f9fafb、次文字 #cfd3d6、错误 #f25a5a 等）
...
- 界面：DSH 默认深色 + 半透明面板（文字容器 0.76–0.94 保可读）
- 无主题装饰（星点/符文/鎏金描边已按用户要求移除）
