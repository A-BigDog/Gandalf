...
### 5.1 配色（v9 最终：**回 DSH 默认深色主题**，不覆盖主题色）
- 界面配色 = DSH 原生 dark palette（用户 2026-08-14 拍板"不要主题色"）
- 仅覆盖表面 token 为**半透明**（空白区 0.08–0.30、文字容器 0.76–0.94），让甘道夫背景图透出
- 文字 token 覆盖为浅色系（#f9fafb 等）——保证任何 color-scheme 模式可读
- 全局字体 token：`--dsw-font-family` → 霞鹭文楷等宽（本地）+ Cinzel（英文）
...
### 5.3 质感（最终定制清单）
- 背景：用户自制朝阳甘道夫图（原图直出——无遮罩、无 sepia、无压暗，JPEG 306KB base64 内联）
- 字体：`--dsw-font-family` 全局统一（中文霞鹭文楷等宽[本地]/英文 Cinzel[自托管]）；代码保持等宽
- 消息：AI 消息圆角气泡卡片（深色 0.85 + 细描边，max-width 85%）；你的消息 DSH 默认无遮罩
- 消息流靠左对齐（贴侧边栏/页面边缘）
- 控件：新会话透明底、回到底部透明、设置面板深色 0.92、发送按钮五芒星（自制 SVG mask）深色底

## 6. 可读性红线（开发工具铁律）
- 正文对比度 ≥ 4.5:1（WCAG AA）——实测 11/11 全过（正文 20.1:1、次文字 10.4:1）
- 文字容器（气泡/代码/输入）不透明度 ≥ 0.76 保可读；空白区透背景（原图 avgLum ~90 偏亮，面板兜底）
- 代码/文字区域不花哨字体
